...
export const login=(email,password,navigate)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector('post','http://localhost:4000/api/v1/user/login',
                {
                    email,
                    password
                }
...
export const sendOTP=(email,navigate)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector("POST",'http://localhost:4000/api/v1/user/sendotp',{
                email
            })
...
export const signUp=(signUpData,otp,navigate)=>{
    return async(dispatch)=>{
        ...
        dispatch(setLoading(true))
        try {
            const response=await apiconnector('POST','http://localhost:4000/api/v1/user/signup',{
                ...signUpData,
                otp
            })
            console.log(response)
