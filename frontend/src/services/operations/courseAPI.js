...
import {setCourse,setEditCourse,setEditSubSection,setStep} from '../../../src/slices/courseSlice'
...
export const addCourse=(formData)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector("POST",'http://localhost:4000/api/v1/course/create-course',formData)
...
export const createSection=(formData)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector('POST','http://localhost:4000/api/v1/course/add-section',formData)
            console.log(response)
            dispatch(setCourse(response.data.updatedCourse))
            //localStorage.setItem('course',JSON.stringify(response.data.updatedCourse))
...
export const updateSection=(formData)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector('PUT','http://localhost:4000/api/v1/course/update-section',formData)
...
export const deleteSection=(courseID,sectionID)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector('DELETE','http://localhost:4000/api/v1/course/delete-section',{
                courseID,
                sectionID
            })
...
export const createSubSection=(course,formData)=>{
    return async(dispatch)=>{
        ...
        try {
            ...
            const response=await apiconnector('POST','http://localhost:4000/api/v1/course/add-sub-section',formData)
            console.log(response.data)
            const response1=await apiconnector('POST','http://localhost:4000/api/v1/course/get-course-details',{courseID:course._id})
...
export const updateSubSection=(formData)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector('PUT','http://localhost:4000/api/v1/course/update-sub-section',formData)
            console.log(response.data)
            dispatch(setCourse(response.data.updatedCourse))
            //localStorage.setItem('course',JSON.stringify(response.data.updatedCourse))
...
export const deleteSubSection=(courseID,sectionID,subSectionID)=>{
    return async(dispatch)=>{
        ...
        try {
            const response=await apiconnector('DELETE','http://localhost:4000/api/v1/course/delete-sub-section',{
                courseID,
                sectionID,
                subSectionID
            })
...
export const createRateAndReview=(courseID,rating,review)=>{
   return async(dispatch)=>{
    ...
    try {
          const response=await apiconnector('POST','http://localhost:4000/api/v1/course/createRating',{
            courseID,
            rating,
            review
          })
...
export const updateUserReview=(courseID,rating,review)=>{
    return async(dispatch)=>{
        const toastID=toast.loading('Adding your review please wait...')
    try {
        const response=await apiconnector('POST','http://localhost:4000/api/v1/course/update-user-review',{
            courseID,
            rating,
            review
        })
...
export const updateUserCourseProgress=(courseID,subSectionID)=>{
    return async()=>{
        ...
        try {
            const response=await apiconnector('POST','http://localhost:4000/api/v1/course/update-course-progress',{
                  courseID,
                  subSectionID
                })
