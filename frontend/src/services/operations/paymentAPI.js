...
export const createOrder=async(courses,navigate,dispatch,userDetails)=>{
    ...
    try {
        ...
        const response=await apiconnector('POST','http://localhost:4000/api/v1/payment/capturePayment',{courses:courses})
...
async function  sendEmail(response1,amount){
    try {
        await apiconnector('POST','http://localhost:4000/api/v1/payment/sendPaymentSuccessEmail',{
...
async function verifyPayment(formData,navigate,dispatch){
    ...
    try {
        const response=await apiconnector('POST','http://localhost:4000/api/v1/payment/verifyPayment',formData)
