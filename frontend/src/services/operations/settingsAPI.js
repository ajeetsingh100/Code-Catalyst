...
export const changeProfilePic=(formData)=>{
   return async(dispatch)=>{
    ...
    try {
        const response=await apiconnector('PUT','http://localhost:4000/api/v1/profile/update-display-picture',formData)
...
export const updateProfile=(formData)=>{
   return async(dispatch)=>{
    ...
    try {
        const response=await apiconnector('PUT','http://localhost:4000/api/v1/profile/update-profile',formData)
        console.log(response)
...
export const changePassword=(formData)=>{
   return async(dispatch)=>{
    ...
    try {
        const response=await apiconnector('POST','http://localhost:4000/api/v1/user/change-password',formData)
        console.log(response)
        dispatch(setUser(response.data.updatedUser))
        //localStorage.setItem('user',JSON.stringify(response.data.updatedUser))
        console.log(response.data)
